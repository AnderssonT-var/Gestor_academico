...
function listaralumnos_del_grado(idgradoencurso){
    ...
    tableAlumnos = $("#alumnos_del_curso").DataTable({
        ...
        "processing": true,
        "ajax": {
            url: "../controlador/docente/get-notes-letter.php",
            type: 'POST',
...
function Reporte_Notas() {
  ...
  var tbls_Reporte = $("#reportExel").DataTable({
    ...
    "processing": true,
    "ajax": {
      "url": "../controlador/docente/list-notes-letter-alls.php",
      "type": "POST",
      "data": {
        gradoID: gradoID,
        cursoID: cursoID
    ...
    "columns": [
      ...
      ...["practica1", "practica2", "practica3", "practica4",
          ...
          "exsamen1", "exsamen2"].map(campo => ({
            "data": campo,
            "render": function (data, type, row, meta) {

                 const colorClass = (data === 'C') ? 'nota-letra-red' : 'nota-letra-blue'; 
              return `<label class="form-control input-sm nota-letra ${colorClass}"> ${data ?? ''} </label>`;
             
            }
          })),

      // Columna de promedio (calculado)
      {
        "data": null,
        "title": "Promedio",
        "render": function (data, type, row, meta) {
          const letras = [
            row.practica1, row.practica2, row.practica3, row.practica4,
            row.trabajo1, row.trabajo2, row.trabajo3, row.trabajo4,
            row.parcial1, row.parcial2, row.parcial3, row.parcial4,
            row.exsamen1, row.exsamen2
          ];
          ...
          const promedioNum = (count > 0) ? (total / count) : 0;
          return `<strong>${numeroALetra(promedioNum)}</strong>`;
        }
      }
    ],
    "language": idioma_espanol,
...
$('#alumnos_del_curso').on('click', '.editar', function() {
   ...
   $.ajax({
           "url": "../controlador/docente/latter_editar_nota.php",
             type: 'POST',
             data:{
                idalumno:idalumno,
                cursoid:cursoid
...
$('#alumnos_del_curso').on('click', '.vernotas', function() {
    ...
    var cursoid  = $("#idcurso").val();
      ...
      $.ajax({
           "url": "../controlador/docente/show-notes-letter.php",
             type: 'POST',
             data:{
                idalumno:idalumno,
                cursoid:cursoid
...
function QuitarNotas(t) {
   ...
   $.ajax({
        url: '../controlador/docente/remove-totes-letter.php',
        type: 'POST',
        data: {
...
/////////////////////////
function validarABC(input) {
  // Convierte a mayúsculas y elimina todo lo que no sea A, B o C
  input.value = input.value.toUpperCase().replace(/[^ABC]/g, '');
}
//////////////////////////////////
...
if (cont1 == 0) {
    cont1++;
       var scntDiv1 = $('#dynamicDiv');
              ...
              $('<div id="divinput"><div class="col-lg-5"><label for="">Práctica 3:</label><input type="text" class="form-control" id="practica3" placeholder="parcial 3" oninput="validarABC(this)"></div><br><div class="col-lg-5"><label for="">Práctica 4:</label><input type="text" class="form-control"  id="practica4" placeholder="parcial 4" oninput="validarABC(this)"> </div><div class="col-lg-2"><br><a class="btn btn-danger btn-sm" class="col-lg-2" href="javascript:void(0)" id="remInput">'+
...
if (cont2 == 0) {
    cont2++;
       var divtrabj = $('#divtrabajo');
              $('<div id="divinput"><div class="col-lg-5"><label for="">Trabajo 3:</label><input type="text" class="form-control" id="trabajo3" placeholder="ingrese Nota" oninput="validarABC(this)" ></div><br><div class="col-lg-5"><label for="">trabajo 4:</label><input type="text" class="form-control"  id="trabajo4" placeholder="ingrese Nota" oninput="validarABC(this)"> </div><div class="col-lg-2"><br><a class="btn btn-danger btn-sm" class="col-lg-2" href="javascript:void(0)" id="remtrabaj">'+'<span class="fa fa-trash" aria-hidden="true"></span> </a></div></div>'+
                ''+
                ''+
              ''+
          '').appendTo(divtrabj);
...
if (cont3 == 0) {
    cont3++;
       var divparcial = $('#divparcial');
              $('<div id="divinput"><div class="col-lg-5"><label for="">Parcial 3:</label><input type="text" class="form-control" id="parcial3" oninput="validarABC(this)"></div><br><div class="col-lg-5"><label for="">Patcial 4:</label><input type="text" class="form-control"  id="parcial4" oninput="validarABC(this)" > </div><div class="col-lg-2"><br><a class="btn btn-danger btn-sm" class="col-lg-2" href="javascript:void(0)" id="remparcial">'+'<span class="fa fa-trash" aria-hidden="true"></span> </a></div></div>'+
                ''+
                ''+
              ''+
          '').appendTo(divparcial);
...
var exsamen2 =$("#exsamen2").val();
 ...
 $.ajax({
        "url": "../controlador/docente/register_notes_letter.php",
        type: 'POST',
        data: {
          gradoID:gradoID,
            curso:curso,
...
$.ajax({
        "url": "../controlador/docente/update-latter-notes.php",
        type: 'POST',
        data: {
            curso :curso ,
            alumnoid:alumnoid,
