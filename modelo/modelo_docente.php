...
function letter_notes($idalumno,$idgrado,$icurso) {
    $sql = "SELECT alumnoid FROM letter_notes WHERE alumnoid='$idalumno' and notasGrado ='$idgrado' and curso = '$icurso'";
    $resultado = 0;  // Variable para almacenar el resultado
...
function register_notes_letter($gradoID,$curso,$alumnoid,$practica1,$practica2,$practica3,$practica4,$trabajo1,$trabajo2,$trabajo3,$trabajo4,$parcial1,$parcial2,$parcial3,$parcial4,$exsamen1,$exsamen2){

    $practica1 = (isset($practica1) && !empty($practica1)) ? $practica1 : '';
    $practica2 = (isset($practica2) && !empty($practica2)) ? $practica2 : '';
    $practica3 = (isset($practica3) && !empty($practica3)) ? $practica3 : '';
    $practica4 = (isset($practica4) && !empty($practica4)) ? $practica4 : '';

    $trabajo1 = (isset($trabajo1) && !empty($trabajo1)) ? $trabajo1 : '';
    $trabajo2 = (isset($trabajo2) && !empty($trabajo2)) ? $trabajo2 : '';
    $trabajo3 = (isset($trabajo3) && !empty($trabajo3)) ? $trabajo3 : '';
    $trabajo4 = (isset($trabajo4) && !empty($trabajo4)) ? $trabajo4 : '';

    $parcial1 = (isset($parcial1) && !empty($parcial1)) ? $parcial1 : '';
    $parcial2 = (isset($parcial2) && !empty($parcial2)) ? $parcial2 : '';
    $parcial3 = (isset($parcial3) && !empty($parcial3)) ? $parcial3 : '';
    $parcial4 = (isset($parcial4) && !empty($parcial4)) ? $parcial4 : '';

    $exsamen1 = (isset($exsamen1) && !empty($exsamen1)) ? $exsamen1 : '';
    $exsamen2 = (isset($exsamen2) && !empty($exsamen2)) ? $exsamen2 : '';


  $sql = "insert into letter_notes(notasGrado,curso,alumnoid,practica1,practica2,practica3,practica4,trabajo1,trabajo2,trabajo3,trabajo4,parcial1,parcial2,parcial3,parcial4,exsamen1,exsamen2)
...
function listar_Notas_Reportes($gradoID,$cursoID){
 ...
 exsamen1, exsamen2 
 from letter_notes
            inner join  alumno on alumno.idalumno = letter_notes.alumnoid
            inner join  grado on grado.idgrado = letter_notes.notasGrado
              inner join  curso on curso.idcurso = letter_notes.curso
            where notasGrado ='$gradoID' and curso = '$cursoID' ";
   $arreglo = array();
      if ($consulta = $this->conexion->conexion->query($sql)) {
        while ($consulta_VU = mysqli_fetch_array($consulta)) {
                        $arreglo['data'][] = $consulta_VU;
...
function latter_editar_notas($idalumno,$cursoid){
   $sql = "SELECT idnotas,practica1, practica2, practica3, practica4, trabajo1, trabajo2, trabajo3, trabajo4, parcial1, parcial2, parcial3, parcial4, exsamen1, exsamen2 FROM letter_notes where alumnoid='$idalumno' and curso='$cursoid' ";
...
function show_letter_notes($idalumno,$cursoid){
   $sql = "SELECT idnotas,practica1, practica2, practica3, practica4, trabajo1, trabajo2, trabajo3, trabajo4, parcial1, parcial2, parcial3, parcial4, exsamen1, exsamen2 FROM letter_notes where alumnoid='$idalumno' and curso='$cursoid' ";
...
function update_latter_notes($idnotas,$curso,$alumnoid,$practica3,$practica4,$trabajo3,$trabajo4,$parcial3,$parcial4, $exsamen1,$exsamen2){
  $sql = "UPDATE letter_notes set practica3='$practica3',practica4='$practica4',trabajo3='$trabajo3',trabajo4='$trabajo4',parcial3='$parcial3',parcial4='$parcial4',exsamen1='$exsamen1',exsamen2='$exsamen2' where idnotas='$idnotas' and alumnoid='$alumnoid' and curso='$curso' ";
...
//FUNCION DE HORARIOS
 ...
 function mostratarHorario_docent($dia,$hora,$idgrado ){
  ...
  function Quitar_Notas($idnotas){
    ...
    function remove_latter($idnotas){
    $sql=   "DELETE FROM letter_notes  WHERE idnotas = '$idnotas'";
      if ($consulta = $this->conexion->conexion->query($sql)) {
        return 1;
