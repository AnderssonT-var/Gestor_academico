...
if (!empty($_SESSION['S_IDUSUARIO'])) {
    ...
    $idforder = htmlspecialchars($_POST['idforder'], ENT_QUOTES, 'UTF-8');
     $log = htmlspecialchars($_POST['log'], ENT_QUOTES, 'UTF-8');
...
if (isset($_FILES['files'])) {
    ...
    for ($i = 0; $i < $log ; $i++) {
        ...
        if (move_uploaded_file($rutaTempArchivo, $rutaDestino)) {
            ...
            $consulta = $file->Registra_Files_Grado_Foders($nombreArchivo, $extension, $fecha, $gradosId,$idforder);
